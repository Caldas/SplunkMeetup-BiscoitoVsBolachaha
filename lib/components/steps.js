            ...
            {!state.stateSelected ? <span className="name">{state.name}</span> :
                <div className="name-input">
                    <select id="team-name">
                        ...
                        {Object.keys(TEAMS).map(team => <option value={team}>{TEAMS[team]}</option>)}
                    </select>
...
export default function renderSteps(state) {
    return <div className="steps">
        ...
        {renderStep(2, state, {active: !!state.nameSubmitted}, state => <div className="shake-it">SHAKE</div>)}
    </div>;
}
...
export function bindStepDomEvents() {
    ...
    $(document).delegate('.steps #team-name', 'change', e => {
        var $select = $(e.currentTarget);
        var val = $select.val();
        if (!val) return;
        State.applyName(val);
